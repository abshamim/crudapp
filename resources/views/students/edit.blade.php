<x-app-layout>
    ...
    <div class="container mx-auto flex justify-center mt-7 px-4 sm:px-6 lg:px-8">
        <div>
            ...
            <form class="w-full max-w-lg" action="{{ route('update', $student->id) }}" method="POST"> @csrf
                <div class="flex flex-wrap -mx-3 mb-6">
                  <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    <label class="text-xs text-gray-700 font-bold mb-4" for="firstName">First Name</label>
                    <input class="crud-input" value="{{ $student->first_name }}" name="firstName" type="text" placeholder="First Name">
                  </div>

                  <div class="w-full md:w-1/2 px-3">
                    <label class="text-xs text-gray-700 font-bold mb-4" for="lastName">Last Name</label>
                    <input class="crud-input" value="{{ $student->last_name }}" name="lastName" type="text" placeholder="Last Name">
                  </div>

                  <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0 mt-3">
                    <label class="text-xs text-gray-700 font-bold mb-4" for="email">Email</label>
                    <input class="crud-input" value="{{ $student->email }}" name="email" type="email" placeholder="Email">
                  </div>

                  <div class="w-full md:w-1/2 px-3 mt-3">
                    <label class="text-xs text-gray-700 font-bold mb-4" for="phone">Phone</label>
                    <input class="crud-input" value="{{ $student->phone }}" name="phone" type="number" placeholder="Phone">
                  </div>
